"""Here comes the fu**ing Pizza Money! — Main entry point."""
import logging
import os
...
# ---------------------------------------------------------------------------
# Logger — reuses the "PizzaMoney" logger (and its file handler) already
# initialised when pizza_trader was imported.
# ---------------------------------------------------------------------------
...
if not GOOGLE_API_KEY:
    logger.critical("GOOGLE_API_KEY not found in environment variables — aborting.")
...
- Technical Analysis: Comprehensive summary including specific price levels, recent momentum indicators,
  relevant technical signals (such as RSI, MACD, CCI, trend direction), major patterns from the last week,
  and any observable volume or options anomalies. Clearly explain all signals referenced.
- News & Sentiment: Detailed overview of the most important recent news (from the last 7 days), including
  major events, analyst upgrades/downgrades, earnings, product releases, or significant controversies.
  Explicitly note the sentiment (positive, negative, neutral), relevant catalysts, or anticipated volatility.
- Final Recommendation: Precise trading stance ("Buy", "Hold", "Sell", "Short") and a brief rationale.
  Explicitly state a recommended entry price, based on current market data or technical setups, and ensure
  alignment with subsequent stop loss and take profit values.
- Stop Loss: Specific price level(s) where an immediate exit is advised to limit risk. Justify the stop
  placement based on chart levels, volatility, or observed technical support.
- Take Profit: Specific price level(s) to target for profit taking, clearly justified based on resistance
  levels, average move, or anticipated catalyst effect.
...
OUTPUT FORMAT:
Return ONLY the answer as a table in **pipe-delimited format** (Markdown table), with no extra text, no
comments, no introductions, and no explanations. Always use the following headers in English:
...
def main(user_question: str) -> None:
    """Run the Pizza Trader agent for *user_question* and print the report."""
    logger.info("main() called with question: %s", user_question)

    if not user_question.strip():
        logger.warning("Empty question received — exiting.")
        print("Thank you for using Pizza Trader")
        return

    logger.info("Initializing MarketAnalyzer and Agent…")
    ...
    logger.info("Agent completed. Rendering table…")
